...
{       Corrected 1993 by Tsoorgaev A. V. (TL)          }
{                                                       }
{*******************************************************}
...
uses Objects, Drivers, Views, Dialogs, Dos,TVCC;
...
type
  ...
  TFileDialog = object(TTVCCDialog)
...
type
  ...
  TChDirDialog = object(TTVCCDialog)
    DirInput: PInputLine;
    DirList: PDirListBox;
    OkButton: PButton;
    ChDirButton: PButton;
...
uses App, Memory, HistList, MsgBoxA, CaseConv; {TL} {UpCase improvement}
...
{  Control := New(PLabel, Init(R, '~F~iles', FileList));
  ...
  begin
    Insert(New(PTVCCButton, Init(R, '~O~pen', cmFileOpen, Opt)));
    Opt := bfNormal;
    Inc(R.A.Y,3); Inc(R.B.Y,3);
  end;
  if AOptions and fdOkButton <> 0 then
  begin
    Insert(New(PTVCCButton, Init(R, 'O~K~', cmFileOpen, Opt)));
    Opt := bfNormal;
    Inc(R.A.Y,3); Inc(R.B.Y,3);
  end;
  if AOptions and fdReplaceButton <> 0 then
  begin
    Insert(New(PTVCCButton, Init(R, '~R~eplace',cmFileReplace, Opt)));
    Opt := bfNormal;
    Inc(R.A.Y,3); Inc(R.B.Y,3);
  end;
  if AOptions and fdClearButton <> 0 then
  begin
    Insert(New(PTVCCButton, Init(R, '~C~lear',cmFileClear, Opt)));
    Opt := bfNormal;
    Inc(R.A.Y,3); Inc(R.B.Y,3);
  end;
  Insert(New(PTVCCButton, Init(R, 'Cancel', cmCancel, bfNormal)));
  Inc(R.A.Y,3); Inc(R.B.Y,3);
...
begin
  ...
  if TTVCCDialog.Valid(Command) then
  begin
    GetFileName(FName);
...
begin
  R.Assign(16, 2, 64, 20);
  TTVCCDialog.Init(R, 'Change Directory');
