...
{ Abstract TValidator object }
  ...
  TValidator = object(TObject)
    ...
    function IsValid(const S: string{;DoUpCase:Boolean}): Boolean; virtual;
...
{ TPXPictureValidator }
  ...
  TPXPictureValidator = object(TValidator)
    ...
    function IsValidInput(var S: string;
      SuppressFill: Boolean): Boolean; virtual;
    function IsValid(const S: string{;IsDoUpCase:Boolean}): Boolean; virtual;
    function Picture(var Input: string;
      AutoFill: Boolean{;IsDoUpCase:Boolean}): TPicResult; virtual;
    procedure Store(var S: TStream);
  end;
...
function TValidator.IsValid(const S: string{;DoUpCase:Boolean}): Boolean;
begin
  IsValid := True;
  DoUpCase:=True;
end;
...
begin
  ...
  S := '';
  if Picture(S, False{,True}) <> prEmpty then
    Status := vsSyntax;
end;
...
begin
  IsValidInput := (Pic = nil) or
     (Picture(S, (Options and voFill <> 0)  and not SuppressFill{,True}) <> prError);
end;

function TPXPictureValidator.IsValid(const S: string{;IsDoUpCase:Boolean}): Boolean;
var
  Str: String;
  Rslt: TPicResult;
begin
  Str := S;
  Rslt := Picture(Str, False{,IsDoUpCase});
...
function TPXPictureValidator.Picture(var Input: string;
  AutoFill: Boolean{;IsDoUpCase:Boolean}): TPicResult;
var
  I, J: Byte;
  Rslt: TPicResult;
  Reprocess: Boolean;
    ...
    begin
      ...
      begin
        ...
        case Pic^[I] of
          ...
          If DoUpCase Then
           Begin
             If TrueUpCase(Pic^[I]) <> TrueUpCase(Ch) Then
               Begin
                If Ch = ' ' Then
                   Ch := Pic^[I]
                Else
...
function TFilterValidator.IsValid(const S: string{;DoUpCase:Boolean}): Boolean;
var
  I: Integer;
begin
  I := 1;
...
function TRangeValidator.IsValid(const S: string{;DoUpCase:Boolean}): Boolean;
var
  Value: LongInt;
  Code: Integer;
begin
  IsValid := False;
  if inherited IsValid(S{,DoUpCase}) then
  begin
    Val(S, Value, Code);
...
function TLookupValidator.IsValid(const S: string{;DoUpCase:Boolean}): Boolean;
begin
  IsValid := Lookup(S);
end;
