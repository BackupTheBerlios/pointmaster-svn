...
If Not _flgLogOpened Then
       ...
       If IOResult<>0 Then
         Begin
          If StrUp(GetVar(LanguageTag.Tag,_varNONE))=RussianTag Then
           Begin
            WriteLn('Hе могy откpыть файл: '+LogName);
            WriteLn('Лог пеpенапpавлен в NUL');
           End
          Else
           Begin
  ...
  _flgLogOpened:=True;
  {$I-}
  WriteLn(Log,'┌[BEGIN]─['+GetDateString+']─['+GetTimeString+']─['+PntMasterVersion+']');
  {$I+}
  If IOResult<>0 Then;
  If Not MODE_NOCONSOLE Then
    Begin
      {$I-}
      WriteLn('┌[BEGIN]─['+GetDateString+']─['+GetTimeString+']─['+PntMasterVersion+']');
      {$I+}
      If IOResult<>0 Then;
    End;
  End;
      ...
      Begin
       ...
       If IOResult<>0 Then
         Begin
          If StrUp(GetVar(LanguageTag.Tag,_varNONE))=RussianTag Then
           Begin
            WriteLn('Hе могy откpыть файл: '+Logger.LogName);
            WriteLn('Лог пеpенапpавлен в NUL');
           End
          Else
           Begin
...
{$IFDEF SPLE}
{$I-}
WriteLn(Log,'┌[BEGIN]─['+GetDateString+']─['+GetTimeString+']─['+SpleVersion+']');
{$I+}
If IOResult<>0 Then;
{$ELSE}
{$I-}
...
If Not _flgLogOpened Then
 ...
 {$IFDEF SPLE}
 {$I-}
 WriteLn(Log,'└[END]─['+GetDateString+']─['+GetTimeString+']─['+SpleVersion+']');
 {$I+}
 If IOResult<>0 Then;
 {$ELSE}
 {$I-}
 WriteLn(Log,'└[END]─['+GetDateString+']─['+GetTimeString+']─['+PntMasterVersion+']');
 {$I+}
 If IOResult<>0 Then;
 {$ENDIF}
 {$IFNDEF SPLE}
 If Not MODE_NOCONSOLE Then
    Begin
     {$I-}
     WriteLn('└[END]─['+GetDateString+']─['+GetTimeString+']─['+PntMasterVersion+']');
     {$I+}
     If IOResult<>0 Then;
    End;
 {$ENDIF}
